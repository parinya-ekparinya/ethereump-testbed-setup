...
if __name__ == "__main__":
    ...
    def get_chain_by_head(block):
        if block["number"] <= common_ancestor["number"] - 1:
            return []
        elif block["transactions"]:
            return [block]
        else:
    ...
    attacker_legit_contribution = [b for b in chain_odd if b["miner"] == coinbases["attacker"]]

    print "{},{},{},{},{},{},{},{},{},{},{},{}".format(
            args.index, args.time, duration,
            len(chain_odd), len(chain_even),
            len(blocks_by_odd), len(blocks_by_even), len(blocks_by_attacker), len(attacker_legit_contribution),
            diff_base, diff_odd, diff_even)
